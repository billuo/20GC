...
const FROG_ICON_SCENE := preload("res://scene/ui/frog_icon.tscn")
...
func _ready() -> void:
	...
	for i in range(total_lives):
		%LivesCounter.add_child(FROG_ICON_SCENE.instantiate())
	%LivesCounter.move_child(goal_counter, -1)
...
func _physics_process(delta: float) -> void:
	if timer_running:
		time_elapsed += delta
		%Time.text = "%.2f" % time_elapsed


func next_player(last_has_finished: bool):
	var icon: FrogIcon = %LivesCounter.get_child(live_idx)
